...
export default function ThemeControl() {
	...
	const getThemeColor = useCallback(() => {
		isCookie('theme')
			? document.body.style.setProperty('--subPoint', document.cookie.split('theme=')[1].split(';')[0])
			: document.body.style.setProperty('--subPoint', getComputedStyle(document.body).getPropertyValue('--subPoint'));
		inputEl.current.value = document.body.style.getPropertyValue('--subPoint');
	}, [isCookie]);
