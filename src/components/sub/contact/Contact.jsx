...
export default function Contact() {
	...
	const mapInfo = useRef([
		{
			...
			imgPos: {
				offset: new kakao.current.maps.Point(116, 99)
			}
		},
		{
			title: '넥슨 본사',
	...
	const roadview = useCallback(() => {
		new kakao.current.maps.RoadviewClient().getNearestPanoId(
			...
			50,
			panoId => {
				new kakao.current.maps.Roadview(
					viewFrame.current
				).setPanoId(panoId, mapInfo.current[Index].latlng);
			}
		);
	}, [Index]);
	...
	useEffect(() => {
		View &&
			viewFrame.current.children.length === 0 &&
			roadview();
	}, [View, roadview]);
